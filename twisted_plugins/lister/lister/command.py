...
def get_plugins_info(plugins_type):
    ...
    if plugins_type == 'input': 
        plugins_getter = plug.get_input_plugins
    else:
        plugins_getter = plug.get_output_plugins
    for p in plugins_getter():
        yield "%s\t\t%s" % (p.name, p.desc)
